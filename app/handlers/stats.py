...
async def payment_stats_callback(call: CallbackQuery, session: AsyncSession) -> None:
    ...
    stats_message: str = (
        ...
        f"💰 <b>Total revenue XTR:</b> <code>{stats.get('total_revenue_xtr')}</code>\n"
        f"💵 <b>Today revenue XTR:</b> <code>{stats.get('revenue_today_xtr')}</code>\n\n"
        f"💰 <b>Total revenue USD:</b> <code>{stats.get('total_revenue_usd')}</code>\n"
        f"💵 <b>Today revenue USD:</b> <code>{stats.get('revenue_today_usd')}</code>"
    )
